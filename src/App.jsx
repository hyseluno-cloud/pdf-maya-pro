...
// External script loader for PDF-Lib and PDF.js
const loadScripts = () => {
  return Promise.all([
    new Promise((resolve) => {
...
const App = () => {
  ...
  const renderAndAnalyze = async (pageNumber) => {
    ...
    } catch (err) { showStatus('error', 'Error processing PDF.'); }
  ...
  const handleFileUpload = async (e) => {
    ...
    } catch (err) { showStatus('error', 'Upload error.'); }
    setLoading(false);
  };
  ...
  const saveTextEdit = async () => {
    ...
    } catch (err) { showStatus('error', 'Error al guardar.'); }
    setLoading(false);
  };
    ...
    >
      ...
      <nav className="bg-white border-b px-3 md:px-8 py-3 flex items-center justify-between sticky top-0 z-[60] shadow-sm mobile-header-container">
        <div className="flex items-center gap-1 md:gap-2">
          ...
          <span className="font-black text-[10px] md:text-xs tracking-widest uppercase hidden xs:block text-slate-800">PDF MAYA PRO</span>
        </div>
        {pdfBytes && (
          <button 
      ...
      {!pdfBytes ? (
        <div className="flex-1 flex flex-col items-center justify-center px-6 text-center">
          <div className="bg-white p-8 md:p-10 rounded-[2.5rem] md:rounded-[3rem] shadow-2xl border border-slate-100 w-full max-w-sm">
             ...
             <p className="text-slate-400 text-[10px] mb-8 font-bold uppercase tracking-widest leading-loose text-center">Edita textos y firma digitalmente.</p>
             <button onClick={() => fileInputRef.current.click()} className="w-full bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl py-10 md:py-12 flex flex-col items-center gap-4 hover:border-indigo-400 hover:bg-indigo-50 transition-all">
                ...
                <span className="font-black text-[9px] md:text-[10px] text-slate-500 tracking-widest uppercase text-center">Elegir Archivo</span>
             </button>
          </div>
        ...
        <>
          {/* Toolbar with mobile optimizations */}
          ...
          <div className="flex-1 overflow-auto bg-slate-200 flex justify-center p-1 sm:p-8 custom-scrollbar">
            <div className="relative shadow-2xl bg-white h-fit transition-transform duration-200 ease-out origin-top" style={{ width: canvasRef.current ? canvasRef.current.width / (window.devicePixelRatio || 1) : '100%' }}>
              ...
              {mode === 'edit' && detectedText.map((item, i) => (
                <div key={i} className={`absolute border-2 border-transparent hover:border-indigo-400/50 hover:bg-indigo-500/5 cursor-text transition-all ${editingIndex === i ? 'border-indigo-600 bg-white/10 ring-8 ring-indigo-500/10 z-40' : ''}`}
                  style={{ left: item.x - 2, top: item.y - 2, width: item.width + 4, height: item.height + 4 }}
                  onClick={(e) => { e.stopPropagation(); setEditingIndex(i); setTempText(item.str); }}>
                  {editingIndex === i && (
                    <div className="absolute -top-16 left-0 flex flex-col sm:flex-row gap-2 bg-white p-3 rounded-2xl shadow-2xl border border-slate-200 z-[100] min-w-[240px]" onClick={e => e.stopPropagation()}>
              ...
              {/* Adjustable Signature Placement */}
              {mode === 'place_sign' && signatureImg && (
                ...
                >
                  <img src={signatureImg} className="w-full h-full object-contain pointer-events-none" alt="Signature preview" />
                ...
                <div className="fixed inset-0 z-[100] bg-slate-900/50 backdrop-blur-md flex items-center justify-center p-4">
                   <div className="bg-white p-6 rounded-[2.5rem] shadow-2xl w-full max-sm border border-slate-100 animate-in fade-in zoom-in duration-300">
                      <div className="flex justify-between items-center mb-6">
                        <h4 className="font-black text-[9px] text-slate-400 uppercase tracking-[0.2em] text-center">Panel de Firma</h4>
